function AddEdit() {
    ...
    $.ajax({
        type: 'POST',
        url: $('head base').attr('href') + 'GameAttemptdata/AddEdit',
        data: formData,
        cache: false,
        contentType: false,
        processData: false,
        ...
        success: function (response) {
            ...
            if (response != '') {
                ...
                if (response.data == 1 || response.data == 2) {
                    ...
                    clear();
                    if (response.data == 2) {
                        window.location.href = $('head base').attr('href') + "GameAttemptdata/Create";
                    }
                    else {
                        window.location.href = $('head base').attr('href') + "GameAttemptdata/Index";
                    }
                }
                else {
...
function FillChallenge() {
    ...
    $.ajax({
        url: $('head base').attr('href') + 'GameAttemptdata/FillChallenge',
        type: "GET",
        dataType: "JSON",
        data: { idGame: Id_Game },
