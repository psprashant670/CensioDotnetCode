...
{
    [SessionTimeout]
    public class Game5AttemptdataController : Controller
    {
        ...
        public Game5AttemptdataController(IUnitOfWork unitOfWork, db_censio_betaContext context, IWebHostEnvironment hostEnvironment)
        {
        ...
        public ActionResult Index()
        {
            var model = (from T in this.DbContext.TblGameAttemptdata
                         ...
                         where T.IdOrganization == ID_ORGANIZATION && T.IdGame==5
                         select new
                         {
                         ...
                         }).Select(X => new GameAttemptdataModel
                         {
                             ...
                             Game5Behaviors = X.Game5Behaviors,
                         }).ToList();

            ViewBag.PageName = "Game5 Attemptdata List";
        ...
        public ActionResult Edit(int? id)
        {
            ...
            else
            {
                ...
                model.Game5Behaviors = tblData.Game5Behaviors;
            }

            ViewBag.Game5Behaviors = model.Game5Behaviors;

            TempData["Behaviors"] = new SelectList(DbContext.TblBehaviorElement.Where(x => x.Status == "A").ToList(), "IdBehavior", "BehaviorElement");
            ViewBag.modelTitle = "Edit"; ViewBag.gameid = model.IdGame;
            

            model.GameList = DbContext.TblGameMaster.Where(x=>x.IdGame==5).ToList().Select(X => new SelectListItem
            {
                Text = X.GameName,
